class VotingPoll < ActiveRecord::Base
  ...
  def VotingPoll.unvoted_for_all
    ...
    polls = VotingPoll.limit(5).order("#{VotingPoll.table_name}.created_on DESC")
	    .where("#{VotingPoll.table_name}.project_id in (#{User.current.projects.collect{|m| m.id}.join(',')}) and vv.id is null")
  ...
  def VotingPoll.unvoted(project_id)
    polls = VotingPoll.limit(5).order("#{VotingPoll.table_name}.created_on DESC")
	    .where("#{VotingPoll.table_name}.project_id = (#{project_id}) and vv.id is null")
	    .joins("LEFT JOIN #{VotingVote.table_name} vv
                                           ON #{VotingPoll.table_name}.id = vv.poll_id
                                              and vv.user_id = #{User.current.id} ")
    return polls;
